...
def index():
    ...
    diff = int((next_send - now).total_seconds()) * 1000  # milliseconds for JS
    ...
    return f"""<!DOCTYPE html>
                ...
                <style>
                    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
                    body {{ font-family: system-ui, sans-serif; background: #f9f9f7; color: #1a1a1a; min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 2rem 1rem; }}
                    .page {{ max-width: 480px; width: 100%; text-align: center; }}
                    .label {{ font-size: 12px; color: #888; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 0.75rem; }}
                    hr {{ border: none; border-top: 1px solid #e5e5e5; margin: 1.25rem 0; }}
                    .word {{ font-size: 44px; font-weight: 500; margin: 0.5rem 0; }}
                    .badge {{ display: inline-block; font-size: 12px; padding: 3px 12px; border-radius: 20px; background: #efefef; color: #666; margin-bottom: 1.75rem; }}
                    .card {{ background: white; border: 1px solid #e8e8e8; border-radius: 12px; padding: 1.25rem 1.5rem; text-align: left; margin-bottom: 1rem; }}
                    .card-label {{ font-size: 11px; color: #999; margin-bottom: 0.4rem; text-transform: uppercase; letter-spacing: 0.06em; }}
                    .card-text {{ font-size: 15px; line-height: 1.65; color: #222; }}
                    .timer-wrap {{ margin-top: 2.5rem; padding-top: 1.5rem; border-top: 1px solid #e5e5e5; }}
                    .timer-label {{ font-size: 12px; color: #999; margin-bottom: 0.5rem; }}
                    .timer {{ font-size: 32px; font-weight: 500; font-variant-numeric: tabular-nums; letter-spacing: 0.04em; }}
                </style>
                </head>
                <body>
                <div class="page">
